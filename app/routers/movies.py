...
# Listar películas con paginación y filtro opcional por título
...
# Eliminar película (delete definitivo)
...
def import_popular_movies(db: Session = Depends(get_db)):
    ...
    for item in data.get("results", []):
        ...
        movie = models.Movie(
            ...
            release_date=release_date,  # ✅ convertido a date
...
def import_movie(tmdb_id: int, db: Session = Depends(get_db)):
    ...
    if release_date_str:
        try:
            ...
            release_date = None  # en caso de formato inválido

    movie = models.Movie(
        tmdb_id=data.get("id"),
        title=data.get("title", "Sin título"),
        overview=data.get("overview"),
        release_date=release_date,  # ✅ ahora es date
